...
class KinematicsModel:
    ...
    def _parse_xml(self):
        ...
        def _parse_joint_limits(joint_node):
            """Parse joint range; support continuous joints without explicit range."""
            range_str = joint_node.attrib.get("range")
            if range_str is None:
                # Continuous joints in MuJoCo often omit range.
                return np.array([-1e6, 1e6], dtype=float)

            dof_limits = np.fromstring(range_str, dtype=float, sep=" ")
            if dof_limits.shape[0] != 2:
                return np.array([-1e6, 1e6], dtype=float)
            return dof_limits
        ...
        def _add_xml_body(xml_node, parent_index, body_index):
            ...
            if body_index == 0:
                ...
                if num_joints == 0:
                    ...
                    _dof_limits = _parse_joint_limits(curr_joints[0])
                    ...
                    for joint in curr_joints:
                        _dof_limits = _parse_joint_limits(joint)
                        self._dof_lower_limits.append(_dof_limits[0])
                        self._dof_upper_limits.append(_dof_limits[1])
                else:
